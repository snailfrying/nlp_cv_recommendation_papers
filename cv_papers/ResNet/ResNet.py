...
# 定义 resnet 18 34 layers的结构
def identity_block(x, filters, kernel_size=3, stride=1, conv_shortcut=False, name=None):
...
# 定义 resnet 50 101 152 layers的结构
...
def identity_stack(x, filters, blocks, stride1=2, conv_shortcut=False, name=None):
    x = identity_block(x, filters, stride=stride1, conv_shortcut=conv_shortcut, name=name)
    for i in range(0, blocks):
        x = identity_block(x, filters, stride=1, name=name + '_block' + str(i))
    return x

def conv_stack(x, filters, blocks, stride1=2, conv_shortcut=False, name=None):
    x = conv_block(x, filters, stride=stride1, conv_shortcut=conv_shortcut, name=name)
    for i in range(1, blocks):
        x = conv_block(x, filters, stride=1, name=name + '_block' + str(i))
    return x
...
def ResNet_18(input_shape, classes):
    ...
    model = training.Model(image_inputs, x, name='resnet-18')
    return model
...
def ResNet_50(input_shape, classes):
    ...
    model = training.Model(image_inputs, x, name='resnet-50')
    return model
...
def ResNet_101(input_shape, classes):
    ...
    model = training.Model(image_inputs, x, name='resnet-101')
    return model
...
def ResNet_152(input_shape, classes):
    ...
    model = training.Model(image_inputs, x, name='resnet-152')
    return model
