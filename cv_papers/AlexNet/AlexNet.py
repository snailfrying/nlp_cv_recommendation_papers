...
def AlexNet(input_shape, classes):
    ...
    x = layers.Conv2D(filters=96, kernel_size=(11, 11), strides=(4, 4), activation='relu')(img_input)
    ...
    x = layers.BatchNormalization()(x)
    # 原论文3*3
    x = layers.Conv2D(filters=384, kernel_size=(1, 1), strides=(1, 1), activation='relu', padding="same")(x)
    x = layers.BatchNormalization()(x)
    # 原论文3*3
    x = layers.Conv2D(filters=256, kernel_size=(1, 1), strides=(1, 1), activation='relu', padding="same")(x)
    x = layers.MaxPool2D(pool_size=(3, 3), strides=(2, 2))(x)
...
if __name__ == '__main__':
    ...
    model = AlexNet(input_shape=input_shape, classes=num_classes)
    # 编译模型
